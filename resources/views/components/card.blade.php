      ...
      <div class="overflow-hidden transition-shadow duration-300 bg-white rounded shadow-sm">
            ...
            <div class="relative">
                ...
                <div class="absolute bottom-10 text-center">
                    ...
                    <div class="absolute left-72 bg-indigo-700 px-2 pb-[2px] rounded-2xl text-center">
                        <p class="text-white text-base font-semibold">${{$item->price}}</p>
                    </div>
                </div>
            </div>
            </div>
        <div class="p-5 border border-t-0">
            ...
            <div class="flex justify-between pb-2 ">
                <p class="bg-indigo-700 text-1xl w-20 h-10 text-white rounded-3xl px-3 md:px-3 md:py-1 font-semibold" >{{$item->category->category_name}}</p>
          ...
          <div class="flex justify-between">
              <a href="{{route('event.show',[$item->id])}}" aria-label="" class="inline-flex items-center font-semibold transition-colors duration-200 text-deep-purple-accent-400 hover:text-deep-purple-800">Learn more</a>
                <form action="{{route('reserver')}}" method="POST">
                    ...
                    <button class="font-medium text-gray-900 hover:bg-indigo-600 px-4 p-1 border-solid border-2 border-gray-400">Reserve</button>
                </form>
          </div>
        </div>
      </div>
...
var nameErro = document.getElementById("nemeErro")

var isValid = true

if(name.trim() === ""){

    nameErro.innerHTML = "please inter your name"
